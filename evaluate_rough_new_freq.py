...
def main(
    ...
    state_dict = torch.load(pt.join(experiment_dir, 'generator_state_dict.pt'), map_location=device)#
    evaluator_new_frequency.G.load_state_dict(state_dict)
    with torch.no_grad():
        size_fake = 5000
    ...
    time = np.linspace(0,1,n_lags)
    ## Store relevant training results
    save_obj(to_numpy(x_real), pt.join(experiment_dir, 'x_real_new_frequency.pkl'))
    ...
    save_obj(to_numpy(x_fake), pt.join(experiment_dir, 'x_fake_new_frequency.pkl'))
    ...
    plt.xlabel("Number of time steps in time discretisation")
    plt.savefig(pt.join(experiment_dir, 'x_real_new_frequency.pdf'))
    plt.close()

    i = 0
    random_indices = torch.randint(0, x_fake.shape[0], (250,))
    plt.plot(to_numpy(x_fake[random_indices, :, i]).T, 'C%s' % i, alpha=0.1)
    plt.xlabel("Number of time steps in time discretisation")
    plt.savefig(pt.join(experiment_dir, 'x_fake_new_frequency.pdf'))
    plt.close()
...
if __name__ == '__main__':
    ...
    # Test run
    benchmark_sigwgan(datasets=('ROUGH',), generators=('LogSigRNN','LSTM'), n_seeds=2, device=device)
    benchmark_wgan(datasets=('ROUGH',), generators=('LogSigRNN','LSTM'), n_seeds=2, device=device)
